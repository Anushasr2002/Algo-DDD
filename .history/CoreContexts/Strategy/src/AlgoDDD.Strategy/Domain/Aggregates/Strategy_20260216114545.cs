...
public class Strategy : AggregateRoot<string>
{
    ...
    public Signal? GenerateSignal(StockSymbol symbol, decimal currentPrice, List<StockData> historicalData)
    {
        if (!IsActive)
            return null;
    ...
    private Signal? GenerateMACrossoverSignal(StockSymbol symbol, decimal currentPrice, List<StockData> historicalData)
    {
        var shortPeriod = Parameters.GetValueOrDefault("ShortPeriod", 10);
        var longPeriod = Parameters.GetValueOrDefault("LongPeriod", 30);
        
        if (historicalData.Count < (int)longPeriod)
            return null;
        ...
        if (shortMA > longMA && shortMA * 0.99m < longMA)
        {
            return new Signal(
                symbol,
                SignalType.Buy,
        ...
        else if (shortMA < longMA && shortMA * 1.01m > longMA)
        {
            return new Signal(
                symbol,
                SignalType.Sell,
    ...
    private Signal? GenerateRSISignal(StockSymbol symbol, decimal currentPrice, List<StockData> historicalData)
    {
        ...
        if (historicalData.Count < (int)period)
            return null;
    ...
    private Signal? GenerateBollingerBandsSignal(StockSymbol symbol, decimal currentPrice, List<StockData> historicalData)
    {
    ...
    private decimal CalculateRSI(List<StockData> data, int period)
    {
    ...
    private (decimal upper, decimal middle, decimal lower) CalculateBollingerBands(List<StockData> data, int period, double stdDevMultiplier)
    {
